...
### biz_register_request — 注册申请

> SQL: `migrations/15_add_register_request.sql`

| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| id | bigint(20) | PK, AUTO_INCREMENT | 主键 |
| email | varchar(50) | NOT NULL | 申请邮箱 |
| note | varchar(500) | DEFAULT NULL | 申请说明 |
| role_key | varchar(20) | NOT NULL | 申请角色: `user` / `reviewer` |
| status | char(1) | DEFAULT '0' | 0=待审 1=通过 2=拒绝 |
| generated_username | varchar(30) | DEFAULT NULL | 审批后生成的用户名 |
| generated_password | varchar(100) | DEFAULT NULL | 审批后生成的密码（BCrypt 加密） |
| review_by | varchar(64) | DEFAULT NULL | 审批人 |
| review_time | datetime | DEFAULT NULL | 审批时间 |
| review_comment | varchar(500) | DEFAULT NULL | 审批意见 |
...
| 序号 | 文件 | 内容 |
|------|------|------|
| 01 | `01_biz_category.sql` | 业务类别表 |
| 02 | `02_add_bill_table.sql` | 票据主表 |
| 03 | `03_add_bill_file_table.sql` | 票据附件表 |
| 04 | `04_add_audit_log_table.sql` | 审核记录表 |
| 13 | `13_add_reviewer_role.sql` | 创建审核员角色 + 分配菜单 |
| 14 | `14_add_admin_role.sql` | 创建系统管理员角色 + 管理菜单 |
| 15 | `15_add_register_request.sql` | 注册申请表 |
| 16 | `16_add_user_remove_perm.sql` | 管理员删除用户权限 |

> 05-12 为图书管理系统旧迁移，已废弃。
...
erDiagram
    ...
    biz_register_request ||--o| sys_user : "审批后创建"
```
