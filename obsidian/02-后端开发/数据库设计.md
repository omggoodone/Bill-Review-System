...
采用单一整合文件 `init-system.sql`，一键初始化全部表结构与种子数据。

```
sql/
└── init-system.sql    # 单一整合文件：全部建表 + 种子数据（角色、字典、菜单、类别、配置）
```

- **新组员**: 直接跑 `init-system.sql` 即可完成全部初始化
- **已有数据库**: 增量变更仍通过 `migrations/` 目录管理
...
`init-system.sql` 是单一整合脚本，包含全部建表语句和种子数据：

- **建表**: 15 张系统表 + 2 张 Quartz 表 + 5 张业务表
- **种子数据**: 角色（admin/user/reviewer/admin_user）、字典、菜单、票据类别、系统配置
- **角色-菜单分配**: 各角色的菜单权限已预先配置完毕
