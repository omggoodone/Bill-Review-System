...
> 本文档与实际代码保持同步，SQL 见 `zsc_backend/sql/init-system.sql`，Java 见 `zsc_module/`。
...
INSERT INTO `sys_role` VALUES
(4, '审核员', 'reviewer', 4, '1', 1, 1, '0', '0', 'admin', NOW(), '', NULL, '票据审核员——查看全部票据，审核通过/退回');

INSERT INTO `sys_role` VALUES
(5, '系统管理员', 'admin_user', 5, '1', 1, 1, '0', '0', 'admin', NOW(), '', NULL, '系统管理员——用户管理、注册审批、统计');
```
...
-- 审核员 (role_id=4): 我的票据 + 票据审核（可查看全部、审核通过/退回，不可新增/编辑/删除）
INSERT INTO `sys_role_menu` (`role_id`, `menu_id`) VALUES
(4, 3000), (4, 3001), (4, 3003), (4, 3002), (4, 3006);

-- 系统管理员 (role_id=5): 系统管理 + 用户管理 + 管理仪表盘（不可审核票据）
INSERT INTO `sys_role_menu` (`role_id`, `menu_id`) VALUES
(5, 1), (5, 100), (5, 101), (5, 102), (5, 103), (5, 104), (5, 105), (5, 106),
(5, 107), (5, 108), (5, 109), (5, 110), (5, 113), (5, 114), (5, 1000),
(5, 1001), (5, 1002), (5, 1003), (5, 1004), (5, 1005), (5, 1006), (5, 1007),
(5, 1008), (5, 1009), (5, 1010), (5, 1011);

-- 超级管理员 (role_id=1): 框架自动放行所有权限，无需显式分配
```
...
校验逻辑:
```
1. 遍历 ids，对每个票据执行单条审核逻辑（同 6.7）
2. 所有票据审核完成后统一返回结果
3. 管理员可批量审核任意已提交状态的票据
```

## 7. 邮件通知流程

注册审批通过后，EmailService (`@Async`) 异步发送邮件通知：

```
approveRegistration(id):
    1. 更新 biz_register_request status=1, 生成 username/password
    2. 创建 sys_user 记录 (BCrypt 加密密码)
    3. 分配角色关联 (sys_user_role)
    4. @Async EmailService.sendAccountCreated(email, username, password)
       → 发送账号密码到用户邮箱
```

账号禁用/删除时同样通过 EmailService 发送通知邮件，不阻塞主流程。
...
├── controller/BizBillController.java          ← 已实现
├── domain/
│   ├── entity/BizBill.java
